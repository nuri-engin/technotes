 ...
 * Account Service
 * 
 * The account service contains the core business logic for account sign up & verification, authentication with JWT & refresh tokens, 
 * forgot password & reset password functionality, as well as CRUD methods for managing account data. The service encapsulates all interaction with 
 * the mongoose account models and exposes a simple set of methods which are used by the accounts controller.
 * 
 * The top of the file contains the exported service object with just the method names to make it easy to see all the methods at a glance, 
 * the rest of the file contains the implementation functions for each service method, followed by local helper functions.
 * 
 */
module.exports = {
    authenticate,
...
async function authenticate({ email, password, ipAddress }) {
    ...
    if (account && !account.isVerified) {
        throw 'Account has not been verified yet!';
    }
...
async function sendVerificationEmail(account, origin) {
    let message = `<p>Please use the token below to verify your email address with the <code>Verify email modal</code>. </p>
    <p>Copy and paste the token into the modal and finalize the registration process.</p>
    <br />
    <p>Your token: <code>${account.verificationToken}</code></p>`
